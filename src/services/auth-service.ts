...
export default class AuthService {
  ...
  static async login(credentials: { employeeId: string; password: string }) {
    const res = await ApiService.post(`${AuthService.baseUrl[0]}/login`, credentials);
    return res;
  }

  static async addUser(credentials: ManageAccountsAddUser) {
    const res = await ApiService.post(`${AuthService.baseUrl[0]}/sign-up`, credentials);
    return res;
  }
  ...
  static async resetPassword({ password, token }: { password: string; token: string }) {
